...
function addUnsigned(a: number, b: number) {
  ...
  if (a4 | b4) {
    return (result & 0x40000000)
      ? result ^ 0xc0000000 ^ a8 ^ b8
      : result ^ 0x40000000 ^ a8 ^ b8
  }
...
function convertToWordArray(input: string) {
  ...
  while (byteCount < messageLength) {
    ...
    wordArray[wordCount] = (wordArray[wordCount] ?? 0) | (input.charCodeAt(byteCount) << bytePosition)
    byteCount += 1
  }
...
export function isMd5Hash(value: string) {
